...
- Added typed `server_definitions` support for validated full and hash-only protocol definitions, including rejection of null or incomplete sections and request-hash matching for hash-only responses. Added XLS-69 `simulate` dry runs with aligned RPC and WebSocket clients, JSON and binary responses, and validated JSON or opaque hexadecimal blob requests. JSON requests support server-autofilled omitted NetworkID values, validate supplied NetworkID values, permit non-empty `SigningPubKey` values and unsigned `Signers` entries, and reject non-empty transaction signatures. Blob requests validate hexadecimal syntax and delegate transaction, signature, and NetworkID checks to the server for compatibility with server-specific definitions. Live RPC and WebSocket integration tests cover full and hash-only definitions plus JSON and binary simulations.
