...
| Change | Action |
| --- | --- |
| Compiler and exported symbols | Update Go, renamed constants, struct fields, and sentinels |
| Exact amounts and fees | Use exact amount helpers and string fee limits |
| Signing and finality | Check canonical signing forms and validated transaction results |
| Network identity | Replace direct field access and update custom servers/test doubles |
| WebSocket lifecycle | Handle disconnects and explicitly resubscribe |
| Batch and MPT | Review the changed payloads and types before reusing stored work |

This is a historical migration guide. Its Go requirement and behavior descriptions refer to `v0.3.0`, not necessarily the latest release. Test upgrades with non-production funds before production deployment.
