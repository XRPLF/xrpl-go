...
The sponsee is the transaction's `Delegate` when present and its `Account` otherwise. X-addresses are converted to classic addresses on a copy before the lookup, because xrpld accepts only classic addresses in the `sponsorship` selector; the caller's transaction is not modified.
