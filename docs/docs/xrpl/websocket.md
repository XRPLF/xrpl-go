...
`WithMaxRetries` limits consecutive reliable-submission monitoring transport failures. For a legal transaction without `LastLedgerSequence`, it also bounds inconclusive polling attempts. It never limits ledger-driven monitoring when `LastLedgerSequence` is present.
