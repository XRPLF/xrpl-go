...
func setupQueryTestClient(t *testing.T, result any) (*Client, func() map[string]any) {
	...
	server := ws.TestWebSocketServer(func(conn *websocket.Conn) {
		...
		// Keep the socket alive until cleanup disconnects the client.
		for {
	...
	setTrustedTestNetworkIdentity(cl, 0)
	t.Cleanup(func() { cl.Disconnect(); server.Close() })
...
func TestClient_GetClosedLedger(t *testing.T) {
	expected := &ledgerqueries.ClosedResponse{LedgerIndex: 123}
...
func TestClient_GetLedger(t *testing.T) {
	expected := &ledgerqueries.Response{LedgerIndex: 123}
