...
var (
	...
	// ErrInvalidSimulateTxBlob is returned when tx_blob is not a non-empty hexadecimal byte string.
	ErrInvalidSimulateTxBlob = errors.New("simulate: tx_blob must be a non-empty hexadecimal byte string")
	// ErrSignedSimulateTransaction is returned when tx_json contains non-empty signature data.
	ErrSignedSimulateTransaction = errors.New("simulate: transaction must be unsigned")
	// ErrInvalidSimulateNetworkID is returned when tx_json NetworkID is not a uint32-compatible number.
	...
	// ErrMismatchedSimulateNetworkID is returned when tx_json NetworkID differs from the client's target network.
