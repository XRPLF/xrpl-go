...
func TestInspectSignedTransaction(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "multisign without optional top-level key", tx: map[string]any{"Signers": []map[string]any{validSigner}}, form: MultiSignedTransaction},
		...
		{name: "empty signers", tx: map[string]any{"Signers": []any{}}, wantErr: true},
		{name: "malformed signers type", tx: map[string]any{"Signers": "invalid"}, wantErr: true},
		{name: "malformed signer wrapper", tx: map[string]any{"Signers": []any{42}}, wantErr: true},
		{name: "malformed signer data", tx: map[string]any{"Signers": []any{map[string]any{"Signer": 42}}}, wantErr: true},
		{name: "signer empty account", tx: map[string]any{"Signers": []any{map[string]any{"Signer": map[string]any{"Account": "", "SigningPubKey": publicKey, "TxnSignature": signature}}}}, wantErr: true},
		{name: "signer wrong account type", tx: map[string]any{"Signers": []any{map[string]any{"Signer": map[string]any{"Account": 42, "SigningPubKey": publicKey, "TxnSignature": signature}}}}, wantErr: true},
		{name: "signer empty public key", tx: map[string]any{"Signers": []any{map[string]any{"Signer": map[string]any{"Account": account, "SigningPubKey": "", "TxnSignature": signature}}}}, wantErr: true},
		{name: "signer wrong public key type", tx: map[string]any{"Signers": []any{map[string]any{"Signer": map[string]any{"Account": account, "SigningPubKey": 42, "TxnSignature": signature}}}}, wantErr: true},
		{name: "signer empty signature", tx: map[string]any{"Signers": []any{map[string]any{"Signer": map[string]any{"Account": account, "SigningPubKey": publicKey, "TxnSignature": ""}}}}, wantErr: true},
		{name: "signer wrong signature type", tx: map[string]any{"Signers": []any{map[string]any{"Signer": map[string]any{"Account": account, "SigningPubKey": publicKey, "TxnSignature": 42}}}}, wantErr: true},
